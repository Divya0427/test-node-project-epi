...
function getUserData(name_array,
  ...
  headerMap = {
    ...
    //An array of columns with which output should be grouped
    var groupByCols = [];
    //Object with groupByCols as a key-value pair groupByColsDataObj
    var groupByColsDataObj = {};
    //An array of groupByColaDataObj
    var groupByArray = [];
    ...
    var DOS = [];
    //E::hasHeader 
    groupby_array && groupby_array.map((arr, index) => {
      ...
      groupByArray.push(groupByColsDataObj);
      colHeaders.push(formattedData[0]);
      if(hasHeader) {
        formattedData.shift();
      }
      userDataByCols.push(formattedData);
      groupByColsDataObj[headerMap[colHeaders[index]]] = userDataByCols[index].filter(Boolean);
    });
    ...
    groupByCols = [];
    if (hasHeader) {
      groupby_array && groupby_array.map(arr => {
        groupByCols.push(headerMap[arr[0][0]]);
      });
    }
    groupByCols = groupByCols || ["name", "memid", "age", "gender", "model", "dos"];
  ...
  //Underscore mixin allows us to extend Underscore with our own utility functions
    ...
    Object.values(_.groupByComposite(result, groupByCols)).map((array) => {
...
function getDataInTabularFormat(name_array,
memid_array,
chartid_array,
age_array,
Gender_array,
...
groupby_array) {
    ...
    var consolidatedOutput = userDataOutput.consolidatedOutput;
    var groupByCols = userDataOutput.groupByCols;
    //tabularUsersData is an array which holds table-like user data
    var tabularUsersData = [];
    Object.values(consolidatedOutput)[0]
    .map(item => {
    ...
    var final_header_value = [...new Set([...groupByCols, ...default_header_value])];
...
function memprofile(
  ...
  var dataInTabularFormat = getDataInTabularFormat(name_array,
    memid_array,
    chartid_array,
    age_array,
    Gender_array,
    ...
    //var tabularOutput = dataInTabularFormat.tabularOutput;
  
  return dataInTabularFormat;
}
...
function dx_profile_compare(
  ...
  groupby_array) {
    /* var usrData = getUserData(
      ...
      hasHeader = true,
      groupby_array);
      var cdngData = usrData.consolidatedOutput[0];
      var merge_cols_list = usrData.groupByCols.filter(Boolean);
    
  var clmsData = getUserData(
      name_array2,
      memid_array2,
      chartid_array2,
      age_array2,
      ...
      hasHeader = true,
      groupby_array
      ).consolidatedOutput[0]; */
      var merge_cols_list = ['name', 'dos', 'chartid'];
      var cdngData = [
        {
          name: "Erik",
          memid: "1",
    ...
    /* New merge functionality starts here */
    ...
    clmsData.map((clmsObj, index, clmsArr) => {
      ...
      if(!isMatchedWithAnyUser) {        
        ...
        console.log('Didnt find the macthed row in table 2');
        var obj = {};
...
cdngData.map((cdngOb) => {
    ...
    cmp.map((obj) => {
      ...
      /*
        obj.dx_increment = dx_increment(dx_1, dx_2, ver, model, age, disabl, never_rump, baserate);
        obj.RAF_increment = dx_increment_raf(dx_1, dx_2, ver, model, age, disabl, never_rump, baserate);
        obj.RAF_gaps = dx_gaps_raf(dx_1, dx_2, ver, model, age, sex, disabl, never_rump, baserate);
        obj.dx_gaps = dx_gaps(dx_1, dx_2, ver, model, age, sex, disabl, baserate);
        console.log(obj); */
    });
    cmp.map((resObj) => {
      var uData = [];
    ...
    /* New merge functionality ends here */
      var tabularOutput = [];
      //tabularOutput.push(tableHeaders);
